...
int IrcServer::ircCommandNick(Command &command) {
  ...
  std::string nickPrefix =
      user && !user->getNickName().empty() ? user->getNickName() : "*";
  ...
  // Validate nickname format (RFC: must begin with a letter or special,max 9
  // chars, no spaces or control characters)
  if (newNick.length() > 9 ||
      newNick.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRST"
                                "UVWXYZ0123456789-[]\\`^{}_") !=
          std::string::npos ||
      isdigit(newNick[0])) {
  ...
  if (!oldNick.empty() && oldNick != newNick) {
    std::string nickMsg = ":" + oldNick + "!" + user->getUserName() + "@" +
