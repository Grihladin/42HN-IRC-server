  ...
  if (recipient[0] == '#') {
    if (params.size() == 1) {
      sendToFd(command.getUserFd(),
               RPL_CHANNELMODEIS(user->getNickName(), recipient,
                                 getChannelByName(recipient)->getMode(), ""));
      return (0);
    }
    ...
    if (channel && channel->isUserOperator(user->getSocketFd())) {
      if (modes[0] == '+') {
        if (modes[1] == 'i')
          ...
          if (channel->isUserOnChannel(params[2].value))
            ...
            sendToFd(command.getUserFd(),
                     ERR_USERNOTINCHANNEL(user->getNickName(), params[2].value,
                                          recipient));
            return (1);
          }
        } else if (modes[1] == 'l')
          ...
          sendToFd(command.getUserFd(),
                   ERR_UNKNOWNMODE(user->getNickName(), modes[0], recipient));
          return (1);
        }
      } else if (modes[0] == '-') {
        if (modes[1] == 'i')
        ...
        sendToFd(command.getUserFd(),
                 RPL_ENDOFBANLIST(user->getNickName(), recipient));
        return (0);
      }
      sendRawMessageToChannel(
          recipient,
          RPL_CHANGE_MODE(user->getNickName(), user->getUserName(),
                          user->getHostName(), command.getRawCommand()));
    } else if (modes[0] == 'b') {
      sendToFd(command.getUserFd(),
               RPL_ENDOFBANLIST(user->getNickName(), recipient));
      return (0);
      ...
      sendToFd(command.getUserFd(),
               ERR_CHANOPRIVSNEEDED(user->getNickName(), recipient));
  } else {
    // MODE for user
    ;
  }
