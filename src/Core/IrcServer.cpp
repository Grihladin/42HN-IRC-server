...
/*   Updated: 2025/10/02 17:04:09 by mratke           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
IrcServer::~IrcServer() {
  if (socket_fds.size() > 0) {
    for (std::vector<struct pollfd>::iterator iter = socket_fds.begin();
         iter != socket_fds.end(); ++iter) {
      close((*iter).fd);
    }
  }
}
...
int IrcServer::openSocket(std::string port) {
  ...
  if (std::stoi(std::string(port)) < 1024 ||
      std::stoi(std::string(port)) > 65535) // check for correct number
  {
  ...
  for (iter = socket_fds.begin(); iter != socket_fds.end(); ++iter) {
    if ((*iter).fd == user_fd) {
      ...
      for (iterBuff = buffers.begin();
           ((*iterBuff).user_fd != user_fd) && (iterBuff != buffers.end());
           ++iterBuff) {
      }
...
std::string IrcServer::getPassword() const { return (password); }

std::string IrcServer::getOperPassword() const { return (operPassword); }
